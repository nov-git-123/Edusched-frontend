...
// WORKING WITHOUT INSTRUCTOR NAME EDIT
...
//         (instructor.course_id || instructor.courseId) === Number(selectedCourseFilter)
//       );
//     }
...
//     return filtered;
//   }, [instructors, courses, searchQuery, selectedCourseFilter]);
...
//           >
//             <option value="all">All Courses ({instructors.length})</option>
//             {courses.map((course) => {
//               const count = instructors.filter(
//                 ins => (ins.course_id || ins.courseId) === course.id
//               ).length;
//               return (
//                 <option key={course.id} value={course.id}>
//                   {course.code} - {course.name} ({count})
//                 </option>
//               );
//             })}
//           </select>
...
//               <thead>
//                 <tr>
//                   <th style={{width: '180px'}}>Instructor Name</th>
...
//WORKING BUT WITHOUT SUCEESFUL MESSAGE AFTER ADD INSTRUCTOR
// import React, { useState, useEffect, useCallback, useMemo } from "react";
// import { Users, Plus, Trash2, Edit, RefreshCw, Search, CheckCircle, XCircle, AlertCircle, X } from "lucide-react";
...
// // 👨‍🏫 INSTRUCTOR ROW COMPONENT
// // ========================================
// const InstructorRow = React.memo(({ instructor, course, onEdit, onDelete }) => {
//   return (
//     <tr className="instructor-row">
//       <td>
//         <div className="instructor-info">
...
//           <div className="time-slots-grid">
//             {SLOT_TIMES.map((time, slotIndex) => (
//               <div key={slotIndex} className="time-slot-box" title={time}>
//                 {time}
//               </div>
//             ))}
//           </div>
...
//       <td>
//         <div className="action-buttons">
//           <button onClick={() => onEdit(instructor)} className="btn-icon btn-edit" title="Edit Instructor">
//             <Edit size={14} />
//           </button>
//           <button onClick={() => onDelete(instructor)} className="btn-icon btn-delete" title="Delete Instructor">
//             <Trash2 size={14} />
//           </button>
//         </div>
//       </td>
...
// // ➕ ADD/EDIT INSTRUCTOR MODAL
// // ========================================
// const InstructorModal = React.memo(({ show, onClose, onSave, instructor, courses, isSubmitting }) => {
//   const [formData, setFormData] = useState({ name: "", courseId: "" });
//   const [errors, setErrors] = useState({});
//   const isEditMode = !!instructor;
...
//       setFormData({
//         name: instructor.name || "",
//         courseId: instructor.courseId || instructor.course_id || ""
//       });
//     } else {
//       setFormData({ name: "", courseId: "" });
//     }
...
//             <div className="warning-box">
//               <AlertCircle size={16} />
//               <span>This instructor has {instructor.schedule.length} scheduled class(es). All schedules will be removed.</span>
//             </div>
//           )}
//           <p className="text-muted">This action cannot be undone.</p>
//         </div>
...
//             ) : (
//               <>
//                 <Trash2 size={16} /> Delete Instructor
//               </>
//             )}
//           </button>
//         </div>
...
//           return {
//             ...ins,
//             courseId: ins.course_id || ins.courseId, // normalize to camelCase
//             schedule
//           };
//         })
//       );
...
//     if (selectedCourseFilter !== "all") {
//       filtered = filtered.filter(instructor => instructor.courseId === Number(selectedCourseFilter));
//     }

//     if (searchQuery.trim()) {
//       filtered = filtered.filter(instructor => {
//         const nameMatch = instructor.name.toLowerCase().includes(searchQuery.toLowerCase());
//         const course = courses.find(c => c.id === instructor.courseId);
//         const courseMatch = course?.code.toLowerCase().includes(searchQuery.toLowerCase());
//         return nameMatch || courseMatch;
//       });
//     }
...
//     try {
//       const isEdit = !!editingInstructor;
//       const url = isEdit ? `${API}/api/instructors/${editingInstructor.id}` : `${API}/api/instructors`;
//       const method = isEdit ? "PUT" : "POST";
...
//         body: JSON.stringify({
//           name: formData.name.trim(),
//           course_id: Number(formData.courseId)  // ← Fixed: snake_case for backend
//         }),
//       });
...
//         ...savedInstructor,
//         courseId: savedInstructor.course_id || savedInstructor.courseId,
//         schedule
//       };

//       if (isEdit) {
//         setInstructors(prev => prev.map(ins =>
//           ins.id === normalizedInstructor.id ? normalizedInstructor : ins
//         ));
//       } else {
//         setInstructors(prev => [...prev, normalizedInstructor]);
//       }

//       setShowInstructorModal(false);
//       setEditingInstructor(null);
//       showToast(`Instructor ${isEdit ? 'updated' : 'added'} successfully!`, "success");
//     } catch (err) {
//       console.error("Save error:", err);
...
//           <div className="page-title-content-instructor">
//             <h1 className="page-title-gradient-instructor">
//               <Users size={36} /> Instructor Management
//             </h1>
//             <p className="page-subtitle-gradient-instructor">
//               Manage faculty members and their teaching schedules
//             </p>
//           </div>
//           <div className="header-actions">
//             <button onClick={handleRefresh} className="action-btn-instructor" disabled={loading} title="Refresh">
//               <RefreshCw size={18} className={loading ? "spinning" : ""} /> Refresh
//             </button>
//             <button onClick={openAddModal} className="action-btn-instructor">
...
//             <option value="all">All Courses ({instructors.length})</option>
//             {courses.map((course) => {
//               const count = instructors.filter(ins => ins.courseId === course.id).length;
//               return (
//                 <option key={course.id} value={course.id}>
//                   {course.code} - {course.name} ({count})
//                 </option>
...
//             <Users size={64} className="empty-icon" />
//             <h3>
//               {searchQuery || selectedCourseFilter !== "all" ? "No instructors match your filters" : "No Instructors Yet"}
//             </h3>
//             <p>
//               {searchQuery || selectedCourseFilter !== "all"
//                 ? "Try adjusting your search or filter criteria"
...
//         }
//         /* Page Header with Gradient */
//         .page-header-instructor { margin-bottom: 2rem; }
//         .page-title-section-gradient-instructor {
//           background: linear-gradient(135deg, ${COLORS.primary} 0%, ${COLORS.accent} 100%);
//           padding: 2rem;
//           border-radius: 16px;
...
//           gap: 1rem;
//         }
//         .page-title-content-instructor { color: white; }
//         .page-title-gradient-instructor {
//           font-size: 2.5rem;
//           font-weight: 700;
//           color: white;
...
//           box-shadow: 0 6px 20px rgba(255, 255, 255, 0.2);
//         }
//         .action-btn-instructor:disabled { opacity: 0.6; cursor: not-allowed; }
...
//           box-shadow: 0 0 0 3px rgba(0, 119, 182, 0.1);
//         }
//         .course-filter-select:hover { border-color: #adb5bd; }
//         .search-box svg {
//           position: absolute;
//           left: 12px;
//           color: #6c757d;
...
//           gap: 16px;
//         }
//         .loading-state p, .error-state p, .empty-state p { color: #6c757d; margin: 0; }
//         .error-state h3 { color: #dc3545; margin: 8px 0; }
//         .error-state svg { color: #dc3545; }
//         .empty-state h3 { color: #03045E; margin: 8px 0; }
//         .empty-icon { color: #0077B6; opacity: 0.5; }
//         .spinner-large {
//           width: 40px;
//           height: 40px;
//           border: 4px solid #f3f3f3;
...
//           transition: all 0.3s ease;
//         }
//         .instructor-row:hover { background: #f8f9fa; }
//         .instructor-row td {
//           padding: 12px 10px;
//           vertical-align: middle;
//           border: 1px solid #dee2e6;
...
//           padding-left: 8px;
//         }
//         .instructor-icon { color: #0077B6; flex-shrink: 0; }
//         .course-badge {
//           display: inline-block;
//           padding: 4px 10px;
//           background: linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%);
...
//           font-weight: 600;
//         }
//         .day-cell { vertical-align: top; padding: 8px 6px !important; }
//         .time-slots-grid {
//           display: flex;
//           flex-direction: column;
//           gap: 3px;
...
//           color: #856404;
//         }
//         .warning-box svg { color: #ffc107; flex-shrink: 0; margin-top: 2px; }
//         .text-danger { color: #dc3545; }
//         .text-muted { color: #6c757d; font-size: 0.9rem; }

//         /* Responsive Design */
//         @media (max-width: 1400px) {
//           .instructor-table { min-width: 1000px; }
//           .day-column { min-width: 120px; }
//           .time-slot-box { font-size: 0.55rem; padding: 2px 3px; }
//         }
//         @media (max-width: 992px) {
//           .page-title-section-gradient-instructor { flex-direction: column; align-items: flex-start; }
//           .header-actions { width: 100%; }
//           .action-btn-instructor { flex: 1; justify-content: center; }
//           .instructor-table { min-width: 900px; }
//         }
//         @media (max-width: 768px) {
//           .instructor-management { padding: 1rem; }
//           .page-title-gradient-instructor { font-size: 2rem; }
//           .controls-bar { flex-direction: column; align-items: stretch; }
//           .search-box, .filter-box { min-width: 100%; }
//           .instructor-table th, .instructor-table td { padding: 8px 6px; font-size: 0.8rem; }
//           .modal-content { width: 95%; max-height: 95vh; }
//           .modal-header-custom, .modal-body, .modal-footer { padding: 20px; }
//           .day-column { min-width: 100px; }
//         }
//         @media (max-width: 576px) {
//           .instructor-management { padding: 0.5rem; }
//           .page-title-gradient-instructor { font-size: 1.5rem; }
//           .instructor-table { min-width: 800px; font-size: 0.75rem; }
//           .instructor-info { flex-direction: column; align-items: flex-start; gap: 4px; }
...
// 👨‍🏫 INSTRUCTOR ROW COMPONENT
// ========================================
const InstructorRow = React.memo(({ instructor, course, onEdit, onDelete, isNewlyAdded }) => {
  return (
    <tr className={`instructor-row ${isNewlyAdded ? 'newly-added' : ''}`}>
      <td>
        <div className="instructor-info">
          <Users size={16} className="instructor-icon" />
          <strong>{instructor.name}</strong>
  ...
  const handleSaveInstructor = useCallback(async (formData) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        setInstructors(prev => [...prev, normalizedInstructor]);
        setRecentlyAddedId(normalizedInstructor.id);
    ...
    <div className="instructor-management">
          ...
          <div className="table-container">
            <table className="instructor-table">
              ...
              <tbody>
                {filteredInstructors.map((instructor) => {
                  const course = courses.find(c => c.id === instructor.courseId);
                  const isNewlyAdded = instructor.id === recentlyAddedId;
                  return (
                    <InstructorRow
                      key={instructor.id}
                      instructor={instructor}
      ...
      <style jsx>{`
        * { box-sizing: border-box; }
        ...
        .instructor-row:hover { background: #f8f9fa; }
        .instructor-row.newly-added {
          background: linear-gradient(90deg, #d4edda 0%, #ffffff 100%);
          animation: successPulse 2s ease;
        }
        @keyframes successPulse {
          0%, 100% { background: linear-gradient(90deg, #d4edda 0%, #ffffff 100%); }
          50% { background: linear-gradient(90deg, #c3e6cb 0%, #f8f9fa 100%); }
        }
        .instructor-row td {
          padding: 12px 10px;
          vertical-align: middle;
