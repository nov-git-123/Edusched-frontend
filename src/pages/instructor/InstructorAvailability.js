...
//WORKING BUT INSTRUCTOR NOT LOGIN ERROR
...
const InstructorAvailability = () => {
  ...
  // Load instructor ID from localStorage - FIXED VERSION
  useEffect(() => {
    const user = JSON.parse(localStorage.getItem("user"));
    console.log("Logged in user:", user);
    
    if (user && user.role === "instructor") {
      // Use email as identifier (consistent with InstructorSchedule)
      const identifier = user.instructorId || user.email;
      setInstructorId(identifier);
    } else {
      showToast("Instructor not logged in. Please log in to manage availability.", "danger");
      setIsFetching(false);
    }
  ...
  const fetchAvailability = useCallback(async () => {
    ...
    try {
      const res = await fetch(`${API}/api/instructor-availability/${encodeURIComponent(instructorId)}`);
