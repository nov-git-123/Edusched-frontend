...
//FUNCTIONAL WITHOUT BTLED MAJOR
...
//         <div className="card-header">
//           <div className="card-course">{row.course}</div>
//           <div className="card-section">{row.section}</div>
//           <div className="card-subject">{row.subject}</div>
//         </div>
//         <div className="card-body">
//           <div className="card-info">
//             <Building size={14} />
...
//   /**
//    * Group schedule by course, section, subject, and room
//    * Consolidates multiple time slots for the same combination
//    */
//   const groupedSchedule = useMemo(() => {
//     const grouped = {};
...
//       const subject = item.subject_code || item.subject_description || "—";
//       const room = item.room_name || "—";
      
//       // Create a unique key combining all four fields
//       const key = `${course}_${section}_${subject}_${room}`;
...
//       if (start && end) {
//         timeRange = `${start} - ${end}`;
//       } else if (item.slot_index !== null && item.slot_index !== undefined) {
//         timeRange = slotToTime(item.slot_index);
//       } else {
//         timeRange = "TBD";
//       }
...
//     // Convert to array and sort by course, then section, then subject
//     return Object.values(grouped).sort((a, b) => {
...
//   /**
//    * Export schedule as image using html2canvas
//    */
...
//         const link = document.createElement('a');
//         link.href = url;
//         link.download = `schedule_${instructorInfo?.name || 'instructor'}_${new Date().toISOString().split('T')[0]}.png`;
//         link.click();
...
//               <h2>Weekly Schedule</h2>
//               <p className="schedule-info">
//                 Showing {groupedSchedule.length} section{groupedSchedule.length !== 1 ? 's' : ''}
//               </p>
//             </div>
//             <ScheduleTable groupedSchedule={groupedSchedule} viewMode={viewMode} />
//           </>
...
// ==================== TIME SLOT CELL ====================
const TimeSlotCell = React.memo(({ timeRange }) => {
  if (!timeRange) {
    return <div className="empty-slot">—</div>;
  }
...
// ==================== SCHEDULE ROW ====================
const ScheduleRow = React.memo(({ row, viewMode }) => {
  const majorLabel = row.major ? BTLED_MAJORS.find(m => m.value === row.major)?.label || row.major : null;
      ...
      <div className="schedule-card-item">
        <div className="card-header">
          ...
          <div className="card-year-sem">
            Year {row.yearLevel} • Sem {row.semester}
          </div>
          <div className="card-subject">{row.subject}</div>
        </div>
        <div className="card-body">
  ...
  return (
    <tr className="schedule-row">
      ...
      <td className="section-cell">
        <div className="section-wrapper">
          {row.section}
          {majorLabel && <span className="major-badge-inline"> ({majorLabel})</span>}
        </div>
      </td>
      <td className="year-sem-cell">
        <div className="year-sem-wrapper">
          Year {row.yearLevel} • Sem {row.semester}
        </div>
      </td>
      <td className="subject-cell">
      ...
      {DAYS_ORDER.map(day => (
        <td key={day} className="day-cell">
          <TimeSlotCell timeRange={row.days[day]} />
        </td>
      ))}
    </tr>
  );
      ...
      <table className="schedule-table">
        <thead>
          <tr>
            <th style={{ width: '150px' }}>Course</th>
            <th style={{ width: '120px' }}>Section</th>
            <th style={{ width: '140px' }}>Year & Semester</th>
            <th style={{ width: '120px' }}>Subject</th>
            <th style={{ width: '100px' }}>Room</th>
            {DAYS_ORDER.map(day => (
              <th key={day} className="day-header">{day}</th>
...
export default function InstructorSchedule() {
  ...
  const groupedSchedule = useMemo(() => {
    ...
    schedule.forEach((item) => {
      ...
      const subject = item.subject_code || item.subject_description || "—";
      const room = item.room_name || "—";
      const yearLevel = item.year_level || "—";
      const semester = item.semester || "—";
      const major = isBTLEDCourse(item.course_code) && item.year_level === 3 ? item.major : null;

      const key = `${course}_${section}_${subject}_${room}_${yearLevel}_${semester}_${major || ''}`;
  ...
  return (
    <div className="instructor-schedule">
      ...
      <div className="page-header">
        ...
        {schedule.length > 0 && (
          <div className="stats-grid">
            <StatsCard icon={BookOpen} label="Subjects" value={stats.subjects} color={COLORS.accent} />
            <StatsCard icon={Building} label="Rooms" value={stats.rooms} color={COLORS.light} />
            <StatsCard icon={Calendar} label="Class Slots" value={stats.slots} color={COLORS.lighter} />
            <StatsCard icon={Clock} label="Total Hours" value={stats.hours} color={COLORS.primary} />
          </div>
        )}
      </div>
