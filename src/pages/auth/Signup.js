...
//WORKING BUT IT DIRECTLY GO TO DASHBOARD
...
//WORKING BUT WITH CONTINUE WITH GOGGLE PROBLEM
...
//   /**
//    * Handle email/password signup
//    * FIXED: Sign out immediately after creating account
//    */
//   const handleSignup = async (e) => {
//     e.preventDefault();
...
//     try {
//       // Set flag to prevent AuthContext from processing auth changes during signup
...
//       // Step 2: CRITICAL - Sign out IMMEDIATELY before doing anything else
//       await signOut(auth);

//       // Step 3: Clear any authentication state
//       localStorage.removeItem('user');
//       sessionStorage.clear();

//       // Step 4: NOW save user to backend database (after sign out)
//       const response = await fetch(`${API}/api/users`, {
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify({
...
//       // Make sure to remove the signup flag on error
//       localStorage.removeItem('signup_in_progress');
//     } finally {
//       setLoading(false);
//     }
...
//   /**
//    * Handle Google OAuth signup
//    * FIXED: Sign out immediately after role selection
//    */
//   const handleGoogleSignup = async () => {
//     setGoogleLoading(true);
//     setErrors({});
...
//     try {
//       // Set flag to prevent AuthContext from processing during signup
//       localStorage.setItem('signup_in_progress', 'true');
...
//       // CRITICAL: Sign out immediately before checking if user exists
//       await signOut(auth);
//       localStorage.removeItem('user');
//       sessionStorage.clear();
      
//       // Now check if user already exists in database
//       const checkResponse = await fetch(`${API}/api/users/${googleUserData.uid}`);
      
//       if (checkResponse.ok) {
//         // User already exists - this is actually a login attempt
//         localStorage.removeItem('signup_in_progress');
//         showToast('Account already exists. Please use login instead.', 'error');
...
//   /**
//    * Handle role selection for Google users
//    * FIXED: Sign out immediately after saving role
//    */
//   const handleSaveRole = async (selectedRole) => {
//     if (!selectedRole || !googleUser) return;
...
//       // Step 2: Close modal and show success
//       setShowRoleModal(false);
//       showToast('Account created successfully! Please log in.', 'success');
...
//         onClose={() => {
//           setShowRoleModal(false);
//           signOut(auth); // Sign out if modal is closed without completing signup
//         }}
//         onSave={handleSaveRole}
//         loading={roleLoading}
//         deanExists={deanExists}
...
//WORKING BUT WITHOUT FULL NAME RESTRICTIONS
...
//   // FIXED: Removed admin role, only show dean and instructor
//   const roles = [
//     { value: 'dean', label: 'Dean', description: 'Faculty management', icon: ShieldCheck, disabled: deanExists },
//     { value: 'instructor', label: 'Instructor', description: 'Teaching access', icon: User, disabled: false },
//   ];
...
//   /**
//    * Check if dean exists on component mount
//    */
//   useEffect(() => {
//     const checkDean = async () => {
//       try {
...
export default function Signup() {
  ...
  const validateFullName = (name) => {
    ...
    // Check for middle initials (single letters between names)
    const hasMiddleInitial = nameParts.some((part, index) => {
      // Skip first and last name, check middle parts
    ...
    if (hasMiddleInitial) {
      return 'Please enter full first and last name without middle initials';
    }
    
    // Check that each name part has at least 2 characters
    const allPartsValid = nameParts.every(part => part.length >= 2);
    if (!allPartsValid) {
  ...
  const handleSignup = async (e) => {
    ...
    // Validate full name with enhanced validation
